...
inline static bool load_file(const TCHAR* path,TCHAR** buffer,unsigned long* size,unsigned long tempsize = 4096)
{
	...
	*buffer = 0;
	...
	if (sizeof(TCHAR) == sizeof(char)) {
		...
		{
			if (strcatgrow(buffer,CString(CStringA(temp))))
			{
				*size += read_bytes;
		...
		TCHAR* sBuf = (TCHAR*) calloc(tempsize, sizeof(TCHAR));
		while(ReadFile(file_handle,(void*)temp,tempsize-1,&read_bytes,0) && read_bytes)
		{
			MultiByteToWideChar(CP_ACP, 0, temp, read_bytes+1, sBuf, tempsize);
			if (strcatgrow(buffer,sBuf))
			{
				*size += read_bytes;
				ZeroMemory(temp,sizeof(char)*tempsize);
			}
...
public:
	...
	//<summary>Cast attribute value as CString. If not found, return empty.</summary>
	//<returns>The CString attribute value, or empty.</returns>
	...
	//<summary>Set attribute to CString.</summary>
	//<param name="rhs">Value CString to set.</param>
	//<returns>Reference to xml_attribute.</returns>
	xml_attribute& operator=(const CString& rhs){ value(rhs); return *this; }
	...
	//<summary>Right-shift attribute value to CString.</summary>
	//<param name="rhs">Reference to CString to set.</param>
	//<returns>Reference to xml_attribute.</returns>
	xml_attribute& operator>>(CString& rhs)
	{
#ifdef PUGOPT_NONSEG
		rhs.clear();
...
//	bool has_name(const CString& name) { return has_name(name); } //Node is named 'name'.
	bool has_attribute(const CString& name) { return has_attribute(name); } //Node has an attribute named 'name'.
#ifdef PUGOPT_NONSEG
...
public:
	...
	xml_attribute attribute(const CString& name){ return attribute(name); }
	//<summary>Access or create the attribute having 'name'.</summary>
	//<param name="name">Name of attribute to access/create.</param>
	//<returns>Reference to xml_attribute wrapper.</returns>
	...
	//</remarks>
	const TCHAR* child_value(CString &value)
	{
		if(_root->children)
		{
...
public:
	...
	xml_attribute_struct* mapto_attribute_ptr(const CString& name){ return mapto_attribute_ptr(name); }
	...
	xml_node_struct* mapto_child_ptr(const CString& name){ return mapto_child_ptr(name); }
	...
	//<returns>Index of child, or -1 if not found.</returns>
	//<remarks>Implement your own hash table if you have a great many children.</remarks>
	int mapto_child_idx(const CString& name){ return mapto_child_idx(name); }
...
public:
	...
	void all_elements_by_name(const CString& name,pointer_array& found){ all_elements_by_name(name,found); }
	...
	//<remarks>xml_node may be invalid if not found; test with 'empty'.</remarks>
	xml_node first_element_by_value(const CString& name,const CString& value){ return first_element_by_value(name,value); }
	...
	//<returns>Valid xml_node if such element named 'name' is found.</returns>
	//<remarks>xml_node may be invalid if not found; test with 'empty'.</remarks>
	xml_node first_element_by_attribute(const CString& name,const CString& attr_name,const CString& attr_value){ return first_element_by_attribute(name,attr_name,attr_value); }
	...
	//<remarks>If matching co-node was found, '_root' points thereto.</remarks>
	bool moveto_first_sibling(const CString& name){ return moveto_first_sibling(name); }
	...
	//<remarks>If matching sub-node was found, '_root' points thereto.</remarks>
	bool moveto_child(const CString& name){ return moveto_child(name); }
	...
	bool moveto_next_sibling(const CString& name){ return moveto_next_sibling(name); }
	...
	CString path(const TCHAR* delimiter = _T("/"))
	{
		...
		strcatgrow(&temp,path); //Append current path.
		free(path); //Free the old path.
		CString returns = temp; //Set path as new string.
		free(temp);
	...
	xml_node first_element_by_path(const CString& path,const CString& delimiter = _T("/")){ return first_element_by_path(path,delimiter); }
...
public:
	...
	//<returns>Success.</returns>
	bool name(const CString& new_name){ return name(new_name); }
	...
	//<returns>Success.</returns>
	bool value(const CString& new_value){ return value(new_value); }
	...
	//<returns>Success.</returns>
	bool remove_attribute(const CString& name){ return remove_attribute(name); }
	...
	xml_attribute append_attribute(const CString& name,const CString& value){ return append_attribute(name, value); }
