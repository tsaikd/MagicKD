...
/////////////////////////////////////////////////////////////////////
long CxDib::WriteBMP(LPCTSTR bmpFileName)
{                         
...
{ UNUSED_ALWAYS(pDC); return 1; }	// doesn't erase the button background
/////////////////////////////////////////////////////////////////////////////
...
void CxShadeButton::SetIcon(UINT nIcon, UINT nIconAlign, UINT nIconDown, UINT nIconHighLight)
{
	...
	SetIcon(m_Icon, nIconAlign, m_IconDown, m_IconHighLight);
}

void CxShadeButton::SetIcon(HICON hIcon, UINT nIconAlign, HICON hIconDown, HICON hIconHighLight)
{
	if (m_IconDown!=m_Icon && m_IconDown) DestroyIcon(m_IconDown);
	if (m_IconHighLight!=m_Icon && m_IconHighLight) DestroyIcon(m_IconHighLight);
	if (m_Icon) DestroyIcon(m_Icon);
	...
	if (m_Icon){							// if success...
		...
		if (GetWindowTextLength()) {
			...
			case BS_RIGHT:
				...
				offX = max(0, x - iinfo.xHotspot*2 - m_FocusRectMargin);
				offY = max(0, (long)(y/2 - iinfo.yHotspot));
				break;
			case BS_LEFT:
				m_IconAlign=BS_LEFT;
				offX = max(0, m_FocusRectMargin);
				offY = max(0, (long)(y/2 - iinfo.yHotspot));
				break;
			default:
				m_IconAlign=BS_CENTER;
				offX = max(0, (long)(x/2 - iinfo.xHotspot));
				offY = max(0, (long)(y/2 - 2*iinfo.yHotspot));
				break;
			}
		} else {
			m_IconAlign=BS_CENTER;
		...
		m_IconBox.OffsetRect(offX, offY);
		...
		if (hIconDown > 0){	//load down icon
			m_IconDown = hIconDown;
			if (m_IconDown==NULL) m_IconDown= m_Icon;
		} else { m_IconDown= m_Icon; }	//reuse resource handle

		if (hIconHighLight > 0){	//load highlighted icon
			m_IconHighLight = hIconHighLight;
			if (m_IconHighLight==NULL) m_IconHighLight= m_Icon;
		} else { m_IconHighLight= m_Icon; }	//reuse resource handle
	}
}
...
{ UNUSED_ALWAYS(wparam); return m_Checked; }	//returns the state for check & radio buttons
/////////////////////////////////////////////////////////////////////////////
...
bool CxShadeButton::SetFont(CString sFontName,long lSize, long lWeight, BYTE bItalic, BYTE bUnderline)
{
	...
	if (m_pLF){
		_tcsncpy(m_pLF->lfFaceName,sFontName,31);
