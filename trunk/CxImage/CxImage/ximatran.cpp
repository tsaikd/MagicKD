		...
		for(long y=0; y<newy; y++){
			...
			for(long x=0; x<newx; x++){
				...
				for(int m=-1; m<3; m++) {
					...
					for(int n=-1; n<3; n++) {
						...
						if (head.biClrUsed){
							...
							cByte  = info.pImage + yy*info.dwEffWidth + xx*3;
							rgb.rgbBlue = *cByte++;
							rgb.rgbGreen= *cByte++;
							rgb.rgbRed  = *cByte;
						}
				...
				if (head.biClrUsed)
					...
					cByte = newImage.info.pImage + y*newImage.info.dwEffWidth + x*3;
					*cByte++ = (BYTE)bb;
					*cByte++ = (BYTE)gg;
					*cByte   = (BYTE)rr;
				}
			...
			for(long y=0; y<newy; y++){
				...
				for(long x=0; x<newx; x++){
					...
					if (head.biClrUsed){
						...
						BYTE* cByte;
						cByte = info.pImage + ifY*info.dwEffWidth + ifX*3;
						rgb1.rgbBlue = *cByte++;	rgb1.rgbGreen= *cByte++;	rgb1.rgbRed =*cByte;
						cByte = info.pImage + ifY*info.dwEffWidth + ifX1*3;
						rgb2.rgbBlue = *cByte++;	rgb2.rgbGreen= *cByte++;	rgb2.rgbRed =*cByte;
						cByte = info.pImage + ifY1*info.dwEffWidth + ifX*3;
						rgb3.rgbBlue = *cByte++;	rgb3.rgbGreen= *cByte++;	rgb3.rgbRed =*cByte;
						cByte = info.pImage + ifY1*info.dwEffWidth + ifX1*3;
						rgb4.rgbBlue = *cByte++;	rgb4.rgbGreen= *cByte++;	rgb4.rgbRed =*cByte;
					}
